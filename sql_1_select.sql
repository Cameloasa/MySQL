city# SQL select query exercise
#
...
SELECT * FROM city;
#
# 3: Get everything on the cities whose district is "aceh" -2
# (Hint: 2 rows)
SELECT *
FROM City 
WHERE District = 'aceh';
#
# 4: Get only the name of the cities where the countrycode is "bfa" - 3 rows
...
#
# 6: Get the name and district named as nm,dist from the cities where the countrycode is "arm" - 3 rows
SELECT Name AS nm, District AS dist 
FROM City 
WHERE countryCode = 'arm';
#
# 7: Get the cities with a name that starts with "bor" - Null
SELECT * 
FROM City 
WHERE Name LIKE 'bor';
#
# 8: Get the cities with a name that contains the string "orto" - Null
SELECT * 
FROM City 
...
WHERE Population < 1000;
#
# 11: Get the cities with the countrycode UKR that has more than 1000000 (one million) in population - 5 rows
SELECT * 
FROM City 
...
#
# 13: Get the cities with the countrycodes TJK, MRT, AND, PNG, SJM - 4 rows
SELECT * 
FROM City 
...
BETWEEN 200 AND 700;
#
# 15: Get the countries with a population between 8000 and 20000 inclusive - 16 rows
SELECT * 
FROM city 
WHERE Population BETWEEN 8000 AND 20000;
#
# 16: Get the name of the countries with a independence year (indepyear) before year 0 - null
SELECT Name 
FROM Coutry 
WHERE IndepYear < 0;
#
# 17: Get the countries that has no recorded independence year and a population above 1000000 - 3 rows
SELECT * 
FROM Country 
